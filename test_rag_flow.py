...
def test_chunk_config_comparison():
    ...
    for name, config in configs.items():
        ...
        print(
            f"  平均长度: {sum(len(c) for c in chunks) // len(chunks) if chunks else 0} 字符"
        )
        print(
            f"  长度范围: {min(len(c) for c in chunks) if chunks else 0} - {max(len(c) for c in chunks) if chunks else 0} 字符"
        )
        print()
