...
from utils.chunk_config import get_default_chunk_config, get_recommended_configs, validate_chunk_config, get_parameters_by_category
...
def test_text_splitter_with_config():
    ...
    configs = [
        ...
        {
            "name": "长文本配置",
            "config": get_recommended_configs()["长文本"]
        }
    ]
...
def test_knowledge_base_with_config():
    ...
    try:
        ...
        configs = [
            ...
            {
                "name": "技术文档配置",
                "config": get_recommended_configs()["技术文档"]
            },
            {
                "name": "自定义配置",
                "config": {
...
def test_config_info_api():
    ...
    print("配置信息:")
    print(f"默认配置: {json.dumps(config_info['default_config'], indent=2, ensure_ascii=False)}")
