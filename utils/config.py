...
TEXT_CHUNK_CONFIG = {
    ...
    "chunk_size": int(os.getenv("TEXT_CHUNK_SIZE", "800")),  # 每个切片的最大字符数，默认800
    "chunk_overlap": int(os.getenv("TEXT_CHUNK_OVERLAP", "100")),  # 相邻切片的重叠字符数，默认100
    
    # 句子切片参数（当split_method为'sentence'时使用）
    "max_sentences_per_chunk": int(os.getenv("MAX_SENTENCES_PER_CHUNK", "3")),  # 每个切片的最大句子数，默认3
    
    # 段落切片参数（当split_method为'paragraph'时使用）
    "paragraph_separator": os.getenv("PARAGRAPH_SEPARATOR", "\n\n"),  # 段落分隔符
    "min_paragraph_length": int(os.getenv("MIN_PARAGRAPH_LENGTH", "30")),  # 最小段落长度（字符数），默认30
    "max_paragraph_length": int(os.getenv("MAX_PARAGRAPH_LENGTH", "1500")),  # 最大段落长度（字符数），默认1500
...
RETRIEVAL_CONFIG = {
    ...
    "similarity_threshold": float(os.getenv("RETRIEVAL_SIMILARITY_THRESHOLD", "0.1")),  # 相似度阈值，默认0.1
    "deduplication": os.getenv("RETRIEVAL_DEDUPLICATION", "true").lower() == "true",  # 是否去重，默认启用
    "retrieval_strategy": os.getenv("RETRIEVAL_STRATEGY", "cosine"),  # 检索策略：cosine, dot_product, euclidean
    "context_window": int(os.getenv("RETRIEVAL_CONTEXT_WINDOW", "1")),  # 上下文窗口大小，默认1
    
    # 权重配置
    "weight_config": {
        "length_weight": os.getenv("RETRIEVAL_LENGTH_WEIGHT", ""),  # 长度权重：prefer_long, prefer_short, ""
        "position_weight": os.getenv("RETRIEVAL_POSITION_WEIGHT", ""),  # 位置权重：prefer_early, prefer_late, ""
        "keyword_weight": os.getenv("RETRIEVAL_KEYWORD_WEIGHT", "").split(",") if os.getenv("RETRIEVAL_KEYWORD_WEIGHT") else [],  # 关键词权重
    }
}
