    ...
    def validate_config(self, config: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        for param_name, value in config.items():
            ...
            if param.depends_on and param.depends_on in config:
                if (
                    config[param.depends_on] != "character"
                    and param.category == "字符数切片"
                ):
                    if param_name not in errors:
                        errors[param_name] = []
                    errors[param_name].append(
                        f"仅在 split_method 为 'character' 时有效"
                    )
                elif (
                    config[param.depends_on] != "sentence"
                    and param.category == "句子切片"
                ):
                    if param_name not in errors:
                        errors[param_name] = []
                    errors[param_name].append(f"仅在 split_method 为 'sentence' 时有效")
                elif (
                    config[param.depends_on] != "paragraph"
                    and param.category == "段落切片"
                ):
                    if param_name not in errors:
                        errors[param_name] = []
                    errors[param_name].append(
                        f"仅在 split_method 为 'paragraph' 时有效"
                    )
