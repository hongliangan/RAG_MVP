...
def retrieve(query, doc_vectors, docs, model_path=None, top_k=3, 
             similarity_threshold=0.0, deduplication=True, 
             retrieval_strategy="cosine", weight_config=None,
             context_window=0):
    """
    基于向量相似度检索相关文档片段。
    ...
    # 应用去重策略
    if deduplication:
        top_indices = _apply_deduplication(top_indices, sims, docs, similarity_threshold)
...
def _apply_weights(sims, docs, weight_config):
    ...
    # 根据文档长度调整权重
    if "length_weight" in weight_config and weight_config["length_weight"] and weight_config["length_weight"] != "":
    ...
    # 根据文档位置调整权重
    if "position_weight" in weight_config and weight_config["position_weight"] and weight_config["position_weight"] != "":
    ...
    # 根据关键词匹配调整权重
    if "keyword_weight" in weight_config and weight_config["keyword_weight"] and len(weight_config["keyword_weight"]) > 0:
    ...
    for idx in indices[1:]:
        ...
        for selected_idx in deduplicated_indices:
            ...
            if selected_idx < len(sims) and idx < len(sims):
                ...
                doc_similarity = min(sims[selected_idx], sims[idx]) / max(sims[selected_idx], sims[idx])
...
def _apply_context_window(indices, window_size, total_docs):
    ...
    for idx in sorted(expanded_indices):
        if idx in indices or any(abs(idx - original_idx) <= window_size for original_idx in indices):
            result.append(idx)
