...
- **rag_core/**：核心功能模块，分为数据加载、向量化、检索、生成、LLM API调用五部分，便于扩展和维护。
    ...
    - **retriever.py**：基于余弦相似度的文档检索，支持top-k检索。
    - **generator.py**：智能拼接prompt并调用LLM生成答案。
    - **llm_api.py**：封装与大语言模型（LLM）API的交互，支持硅基流动等API。
- **utils/**：工具类和配置管理。
    - **config.py**：支持多LLM服务配置（如硅基流动、OpenAI等），可通过环境变量`LLM_PROVIDER`选择服务，参数（API Key、模型名、URL）独立配置，便于灵活扩展。
- **web/**：Web界面模块。
    - **app.py**：Flask应用，提供文件上传和问答界面。
    - **templates/**：HTML模板，支持中文界面和响应式设计。
...
# 3. 检索相关文档
query = "你的问题"
relevant_docs = retrieve(query, doc_vectors, docs)
# 4. 生成答案
